...
{
    public class StepSimulationCalculationByConnectMixerToWIP : MixerStepState
    {
        Amount MaxTime { get; set; } = new(TimeUnits.Minute);
        Amount CurrenTime { get; set; } = new(TimeUnits.Minute);

        public StepSimulationCalculationByConnectMixerToWIP(MixerState mixer, BackBoneStepSimulation stepSimulation) : base(mixer, stepSimulation)
        {
        }
