...
namespace Simulator.Server.EndPoints.Properties
{
    ...
    public static class CreateUpdateCompoundPropertyEndPoint
    {
        public class EndPoint : IEndPoint
        {
            ...
            {
                app.MapPost(StaticClass.CompoundPropertys.EndPoint.CreateUpdate, async (CompoundPropertyDTO Data, IRepository Repository) =>
                {

                    CompoundProperty? row = null;
                    if (Data.Id == Guid.Empty)
                    {
                        row = CompoundProperty.Create();
                    ...
                    .Include(y => y.SuperficialTension);
                        Expression<Func<CompoundProperty, bool>> Criteria = x => x.Id == Data.Id;
                        row = await Repository.GetAsync<CompoundProperty>(Includes: includes, Criteria: Criteria);
